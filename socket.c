...
int main_loop() {
  ...
  for (; run == 1;) {
    ...
    if ((sd = accept(s, (struct sockaddr *)&address, &addr_len)) ==
        -1) { // perror("accept failed");
      continue;
    }
    ...
    if (net_A == 130 && net_B == 216) {
      if (net_C == 32 || net_C == 33 || net_C == 34 || net_C == 35 ||
          net_C == 36 || net_C == 39 || net_C == 104 || net_C == 105 ||
...
void send_error(int sd, int error) {
  ...
  if ((fp = fdopen(sd, "w")) != 0) {
    ...
    if (error == 404) {
      ...
      fprintf(fp,
              "<HEAD><TITLE>%d Error</TITLE>\n<META NAME=\"ROBOTS\" "
              "CONTENT=\"noindex, nofollow, noarchive\">\n</HEAD>\n",
              error);
      fprintf(fp, "<BODY><H1>Invalid Request</H1>\n");
      fprintf(fp, "Your client requested a transmission method other than "
                  "those allowed by this server.\n");
      fprintf(fp, "</BODY>\n");
    }
    fflush(fp);
    fclose(fp);
...
void stats(FILE *fp) {
  ...
  fprintf(fp, "Last Restart: %s<br>",
          asctime_r(gmtime_r(&start_time, &result), time_Buff));
  fprintf(fp, "Current Date: %s<br>",
          asctime_r(gmtime_r(&t, &result), time_Buff));
  ...
  if ((fp = fdopen(sd, "w")) != 0) {
    ...
    } else if (strncmp(url, "/TA", 3) == 0) // create ancestor tree
    {
      ...
      if (p != NULL && *p != '\0' && (g = find_hash(p)) != NULL) {
        ...
        send_error(sd, 404);
    } else if (strncmp(url, "/TD", 3) == 0 &&
               url[3] != 'N') // create descendant tree
    {
      int max_depth = 0;
    ...
    } else if (strncmp(url, "/TDN", 4) ==
               0) // create descendant tree of this name only
    {
      int max_depth = 0;
