...
* Source https://github.com/rbur004/gedserv

## DESCRIPTION:

Simple web server that reads GEDCOM files, and serves them as web pages on port 9999. 

The code is very old, but is still driving www.burrowes.org . The intent is to combine the gedcom ruby gem with a database and finish the interactive html editing code (a Rails application).

Also a ruby rbx helper cgi to add to the external web server.
The ruby cgi is much newer, and replaces an older c based cgi.
