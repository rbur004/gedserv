...
{
char *err_message;
char fudge[65];     /*re_exec returns a match when pattern matches the first part of a string*/
		    /*So we tack on a $ to each string to match the null*/
int l;

	if((l = strlen(p)) > 62)
	{
		printf("Internal error in patternmatch(), length of pattern > 62 chars (%d)\n",l);
		return(0);
	}
	strcpy(&fudge[1],p);
...
char *concat(char *s1, char *s2)  /* return pointer to concatenation of 2 strings */
{
int l1,l2;
...
void padwithspaces(char *s, int size)    /* add ' 's to s until its length is (size-1) */
{
int i;
...
int i;
	...
	strcpy(buff1, f1);
	strcpy(buff2, f2);
	nargs1 = getarglist(buff1, v1, 128); //make arg list for easier matching
	nargs2 = getarglist(buff2, v2, 128);
	if(nargs1 != nargs2)
		return 0;
	else
		for(i = 0; i < nargs1; i++)
