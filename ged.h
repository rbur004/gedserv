...
#define LINE_LIMIT 256 // Standard says 120 for First and Surname combined length
#define REF_LENGTH 32  // Standard says 22, but we want nice buffer sizes

typedef struct rel_list_struct {
  struct rel_list_struct *next;
  struct ged_record *rt;
  struct ged_record *srt;
  int level; // when used as a link to the previous child this has the
             // generation of the current node through this child
  int direct;
} rel_list_type;
...
struct ged_record {
  ...
  rel_list_type
      *rel_child_1; // Keeps track of child we came to this node through.
  rel_list_type
      *rel_child_2; // We need to also track path from second start point.
};
...
typemap_t typemap[] = {
    "INDI", INDI, "FAM",  FAM,  "EVEN",    EVEN, "CSPO", CSPO, "NAME",  NAME,
    "FATH", FATH, "MOTH", MOTH, "FCHI",    FCHI, "YSIB", YSIB, "OSIB",  OSIB,
    "MARR", MARR, "LSPO", LSPO, "ESPO",    ESPO, "SPOU", SPOU, "SEX",   SEX,
    "BIRT", BIRT, "FAMS", FAMS, "FAMC",    FAMC, "HUSB", HUSB, "WIFE",  WIFE,
    "CHIL", CHIL, "HEAD", HEAD, "DATE",    DATE, "PLAC", PLAC, "QUAY",  QUAY,
    "NOTE", NOTE, "DEAT", DEAT, "DIVO",    DIVO, "OCCU", OCCU, "_PICT", PICT,
    "TITL", TITL, "BURI", BURI, "EDUC",    EDUC, "SOUR", SOUR, "REPO",  REPO,
    "CONT", CONT, "RESN", RESN, "CONC",    CONC, "CHR",  CHR,  "WILL",  WILL,
    "ADOP", ADOP, "CREM", CREM, (char *)0, 0};
