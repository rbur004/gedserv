...
int parse_date(char *date) {
  ...
  char *v1[ARGLIST_LEN]; // Split string into arglist
  static char buff1[DATE_BUFF_LEN];
  int nargs1;
  int i;
  int base = 0;
  ...
  strncpy(
      buff1, date,
      DATE_BUFF_LEN); // make a copy as we are going to fill it full of nulls
  buff1[DATE_BUFF_LEN - 1] = '\0';                   // Ensure null termination
  if ((nargs1 = getarglist(buff1, v1, ARGLIST_LEN))) // make arg list
  {
    if (findcasestring(v1[base], Calendar_Escape_Sequence,
                       CALENDAR_ESCAPE_SEQUENCE_SIZE) != -1)
      base++;
    ...
    if (base < nargs1 && isdigit(*v1[base]) && (day = atoi(v1[base])) <= 31 &&
        day > 0)
      base++;
    else {
    ...
    if (base < nargs1 &&
        (month = findcasestring(v1[base], months, NMONTHS)) != -1) {
      if (day == 0)
