...
void delete_line(ged_type *line) {
  ...
  {
    if (line->next &&
        line->last) // Ensure the link list doesn't have a hole in it
    {
    ...
    } else if (type_to_num(line->type) ==
               NAME) // Name are part of a name hash list
    {
...
void add_NAMEhash(ged_type *line) {
  ...
  hash = hash_string((unsigned char *)line->data, 1024);
  line->NAMEhash_link =
      NAMEhash_table[hash];    // Hash collision resolution by chaining
...
ged_type *attach(ged_type *insert, char *level_p, char *type_p, char *data_p) {
  ...
  if ((this_line = malloc(sizeof(ged_type))) == NULL)
    return (ged_type *)0;
  ...
  if (level == 0) {
    // For level zero lines we need to add to hash index and to add to type
    // chains
    add_hash(this_line);
    switch (type_to_num(this_line->type)) {
    case INDI:
      add_list(this_line, &head_INDI);
      break;
    case FAM:
      add_list(this_line, &head_FAM);
      break;
    case EVEN:
      add_list(this_line, &head_EVEN);
      break;
    case SOUR:
      add_list(this_line, &head_SOUR);
      break;
    case NOTE:
      add_list(this_line, &head_NOTE);
      break;
    case REPO:
      add_list(this_line, &head_REPO);
      break;
    }
  } else {
    switch (type_to_num(this_line->type)) {
    case NAME:
      add_NAMEhash(this_line);
      break; // add names to name hashtable
    }
  }
  return this_line;
}

ged_type *dup_data_and_attach(ged_type *insert, int level, char *type,
                              char *data) {
  ...
  if ((buffer = malloc(type_len + data_len)) == NULL)
    return 0;
  strncpy(buffer, type, type_len);
  strncpy(&buffer[type_len], data, data_len);
  buffer[type_len + data_len - 1] = '\0';
...
void parse_gedcom(char *buff, int num_files) {
  ...
  do {
    ...
    if ((level_p = skip_spaces(l)) != 0) { // Line contains something
      if (*level_p == '0') { // This is a level 0 line (We assume no leading
                             // 0's)
      ...
      } else if (isdigit(*level_p)) { // This is a sublevel line
...
ged_type *find_spouse(ged_type *fam, char *indi_reference) {
  ged_type *spouse;
  char spouse_reference[REF_LENGTH];
  ged_type *family;

  family = find_hash(fam->data);
  if ((spouse = find_type(family, HUSB))) {
    strip_ats(spouse_reference, spouse->data);
    if (strncmp(indi_reference, spouse_reference, 32) ==
        0) // Looking at self, not spouse.
    {
      if ((spouse = find_type(family, WIFE)) ==
          (ged_type *)0)      // Look for WIFE instead
        return (ged_type *)0; // Didn't find a spouse.

      strip_ats(spouse_reference, spouse->data);
      if (strncmp(indi_reference, spouse_reference, 32) ==
          0) // Looking at self, not spouse, which is an error in the gedcom
             // file.
        return (ged_type *)0; // Didn't find a spouse.
    }
  } else // no HUSB or WIFE recorded, or just one is recorded, which means it
         // must be self (or a gedcom error)
    return (ged_type *)0; // Didn't find a spouse.
...
void follow_child_list(ged_type *parent, ged_type *child) {
  ...
  if ((first_child = find_type(parent, FCHI)) &&
...
void check_links() {
  ...
  int spouse;
  char *atp1, *atp2;
  char name1[LINE_LIMIT], name2[LINE_LIMIT];

  indicated_sex[1] = '\0';
  for (base_record = head_INDI.next_this_type; base_record != &head_INDI;
       base_record = base_record->next_this_type) // for each INDI record
  {
    ...
    for (g = base_record->next; g->level > 0;
         g = g->next) // For each sub record
    {
      ...
      case FAMS:
        ...
        if ((fam = find_hash(g->data))) {
          ...
          } else if (indicated_sex[0] != '\0' &&
                     *(sex->data) != indicated_sex[0])
  ...
  for (base_record = head_FAM.next_this_type; base_record != &head_FAM;
       base_record = base_record->next_this_type) // for each FAM record
  {
    spouse = 0;
    test_ats(base_record->data);
    husb_name.name = 0;
      ...
      case CHIL:
        ...
        if ((chil = find_hash(g->data))) {
          if ((chil_name.name = find_type(chil, NAME)) && husb_name.name) {
            ...
            //    printf("HUSB %s surname %s of FAM %s doesn't match child %s
            //    surname %s\n",
            //        husb_name.indi->data, base_record->data, name1, g->data,
            //        name2);
          }
...
void check_dates(void) {
       ...
       g = g->next_this_type) // for each INDI record
  {
    ...
    if (birt_date && deat_date &&
        cmp_dates(birt_date->data, deat_date->data) == 1)
      printf("Warning INDI %s died before he was born b. %s d.%s\n", g->data,
             birt_date->data, deat_date->data);
    if (birt_date && marr_date &&
        cmp_dates(birt_date->data, marr_date->data) == 1)
      printf("Warning INDI %s married before he was born b. %s m.%s\n", g->data,
             birt_date->data, marr_date->data);
    if (deat_date && marr_date &&
        cmp_dates(marr_date->data, deat_date->data) == 1)
      printf("Warning INDI %s married after he died m. %s d.%s\n", g->data,
             marr_date->data, deat_date->data);
  }
}
...
int all_present_and_correct(char *indi, char *fam, ged_type *marr,
                            ged_type *fam_marr) {
  ged_type *g;
  ged_type *s;
  ged_type *next;
  // Checks that the sub fields of marr exist and are the same as the subfields
  // of fam_marr Needs work, it reports some diff badly
...
void repo_given(FILE *fp, ged_type *record) {
  ...
  if ((repo = find_type(record, REPO)))
    fprintf(
        fp,
        "<a href=\"/ruby/gedrelay.rbx?type=repo&target=%s\"><b>Source</b></a>",
        repo->data);
}
...
static void source_body(FILE *fp, ged_type *source) {
  ...
  if (*source->data == '@')
    fprintf(fp,
            "&nbsp;&nbsp;&nbsp;&nbsp;<a "
            "href=\"/ruby/gedrelay.rbx?type=sour&target=%s\"><b>Source</b></a>",
            source->data);
  else {
    fprintf(fp, "<br><ul><b>Source.</b>%s\n", source->data);
    for (g = source->next; g && g != &all && g->level > source->level &&
                           ((t = type_to_num(g->type)) == CONT || t == CONC);
         g = g->next)
      if (t == CONC)
        fprintf(fp, "%s\n", g->data);
      else
...
static void note_body(FILE *fp, ged_type *note, int indent) {
  ...
  if (isPrivate(note) == 0) {
    if (indent)
      ...
      fprintf(fp, "<p>");
    fprintf(fp, "<b>Nb.</b> %s\n", note->data);
    for (g = note->next; g && g != &all && g->level > note->level &&
                         ((t = type_to_num(g->type)) == CONT || t == CONC);
         g = g->next)
      if (t == CONC)
        fprintf(fp, "%s\n", g->data);
  ...
  if ((name = find_type(indi, NAME))) {
    if (!privacy) {
      ...
      if ((famc = find_type(indi, FAMC)) && parent) {
        ...
        strcat(buff, ".html");
        fprintf(fp,
                " <a href=\"/ruby/gedrelay.rbx?type=html&target=%s\" "
                "><b>Parents</b></a>. ",
                buff);
      }
      ...
      if (adop && (afamc = find_type(adop, FAMC))) {
        ...
        strcat(buff, ".html");
        fprintf(fp,
                " <a href=\"/ruby/gedrelay.rbx?type=html&target=%s\" ><b>Adopt "
                "Parents</b></a>. ",
                buff);
      }
      ...
      if (famc)
        fprintf(
            fp,
            " Draw <a href=\"/ruby/gedrelay.rbx?type=TA&depth=15&target=%s\"  "
            "><b>Anc.</b></a> Tree. ",
            buff);
      ...
      if ((fams = find_type(indi, FAMS)))
        fprintf(
            fp,
            " Draw <a href=\"/ruby/gedrelay.rbx?type=TD&depth=15&target=%s\"  "
            "><b>Desc.</b></a> Tree.\n",
            buff);
      ...
      if ((birt = find_type(indi, BIRT))) {
        ...
        if ((date = find_type(birt, DATE))) {
          int b_date = 0;
          if (!dead && (b_date = return_year(date->data)) && b_date != 0 &&
              (year - b_date) <= 100)
      ...
      if ((chr = find_type(indi, CHR))) {
        ...
        if ((date = find_type(chr, DATE))) {
          int c_date = 0;
          if (!dead && (c_date = return_year(date->data)) && c_date != 0 &&
              (year - c_date) <= 100)
    ...
    if (fams) {
      ...
      strcat(buff, ".html");
      fprintf(fp,
              "<dd><a href=\"/ruby/gedrelay.rbx?type=html&target=%s\" "
              "><b>M.%d.</b></a>\n",
              buff, mcount++);
      while ((fams = find_next_this_type(indi, fams))) {
        ...
        strcat(buff, ".html");
        fprintf(fp,
                "<a href=\"/ruby/gedrelay.rbx?type=html&target=%s\" ><b>, "
                "M.%d.</b></a>\n",
                buff, mcount++);
      }
      fprintf(fp, "</dd>\n");
...
void Indi_html_dump(FILE *fp, ged_type *g) {
  ...
  fprintf(fp, "<b>Go to the Tree's </b><A "
              "HREF=\"https://www.burrowes.org/FamilyTree/\"  ><b>Entry "
              "Page</b></A>.<br>\n");
  fprintf(fp,
          "Dump raw <A HREF=\"/ruby/gedrelay.rbx?type=ged&target=%s\" "
          ">GEDCOM</a><p>\n",
          buff);
...
void Family_html_dump(FILE *fp, ged_type *g) {
  ...
  if ((pict = find_type(g, PICT))) {
    fprintf(
        fp,
        "<IMG SRC=\"https://www.burrowes.org/FamilyTree/%s\" ALIGN=TOP><br>\n",
...
void load_gedcom(void) {
  ...
  while (Ged_File[i].name) /*work out how much memory we need*/
  {
    if ((fd = open(Ged_File[i].name, O_RDONLY)) != -1) {
      ...
      // while we are at it, set the mod time to the latest mod time for this
      // file.
  ...
  if ((Data_buffer = malloc(file_size + 1)) == NULL) // Alloc the memory
    exit(1);
