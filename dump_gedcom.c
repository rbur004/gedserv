...
void html_dump(void) {
  ...
  for (g = head_FAM.next_this_type; g != &head_FAM;
       g = g->next_this_type) // for each FAM record
  {
    ...
    if ((fp = fopen(buff, "w"))) {
      ...
      fprintf(
          fp,
          "<A HREF=\"/ruby/gedrelay.rbx?type=surname&target=index_A\">Surname "
          "Only Index</A>\n");
      ...
      if ((pict = find_type(g, PICT))) {
        fprintf(fp,
                "<IMG SRC=\"https://www.burrowes.org/FamilyTree/%s\" "
                "ALIGN=TOP><br>\n",
