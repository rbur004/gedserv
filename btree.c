...
// The Left hand branch is deeper than the Right
{
  ...
  case 0: // the left child tree is balanced and we just wasted our time calling
          // the function
    return tree;
...
// The right hand branch is deeper than the left
{
  ...
  case 0: // the right child tree is balanced and we just wasted our time
...
static btree_node_p add_node(btree_node_p tree, tree_data *data) {
  ...
  if (tree ==
      0) { // there is no tree or we are adding to a subtree at a leaf node
    if ((new_node = malloc(sizeof(btree_node))) == NULL) {
  ...
  } else // this isn't a leaf node and we may need to rotate the tree to balance
         // it.
  {
    switch (cmp_node(data, tree->data)) {
    case -1: // New word is before , alphabetically, the one in this node
      tree->left = add_node(
          tree->left, data); // add node to the left side is data < tree->data
      switch (tree->balance) {
      case 1:
    ...
    case 1: // New word is after the one, alphabetically, in this node
      tree->right =
          add_node(tree->right,
                   data); // add node to the right side is data >= tree->data
      switch (tree->balance) {
      case -1:
...
void dump_by_match(FILE *fp, char *match_me) {
  char indi_reference[REF_LENGTH];
  char fam_reference[REF_LENGTH];
  ged_type *name;
  ged_type *fams;
  ged_type *famc;
  ged_type *chr;
  ...
  ged_type *spouse;
  char *spouse_name;
  char firstnames[LINE_LIMIT], surname[LINE_LIMIT];
  int mcount;
  ...
  for (j = 0; j < 26; j++)
    fprintf(fp,
            ...
            ">%c</A> \n",
            j + 'A', j + 'A');
  fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=surname&target=index_?\" "
              ">?</A><br>\n");
  ...
  fprintf(fp, "<p></KBD></pre><b>Go to the Tree's </b><A "
...
static void dump_tree_body(FILE *fp, btree_node_p tree, char *filename,
                           int surname_or_full, char *last_surname,
                           int no_surname) {
  char indi_reference[REF_LENGTH];
  char fam_reference[REF_LENGTH];
  ged_type *fams;
  ...
  if (tree == 0)
    ...
    dump_tree_body(fp, tree->left, filename, surname_or_full, last_surname,
                   no_surname);

    if (no_surname) {
      firstname = 1;
      extract_firstnames(tree->data,
                         surname); // fudge it by using the firstname.
    } else {
      firstname = 0;
    ...
    if (strcasecmp(last_surname, surname) != 0) {
      if (surname_or_full & 0x4) // only want this surname
      {
        ...
        dump_tree_body(fp, tree->right, filename, surname_or_full, last_surname,
                       no_surname);
        return;
      }

      fprintf(fp, "<A NAME=\"%s\"></A>\n", surname);
      strncpy(last_surname, surname, LINE_LIMIT);
      last_surname[LINE_LIMIT - 1] = '\0';
      // surname_count++;

      if (surname_or_full & 0x1)
        fprintf(
            fp,
            "<A HREF=\"/ruby/gedrelay.rbx?type=search&%s=%s\"  >%s</A><br>\n",
    ...
    if (surname_or_full & 0x2) {
      ...
      // Output birth date, if known. Christening date, if known, and birth date
      // not known.
      if ((birt = find_type(tree->data->indi, BIRT)) &&
          (date = find_type(birt, DATE))) {
        if ((b_date = return_year(date->data))) // Yes one ='s
          fprintf(fp, " <b>b.</b>%4.4d", b_date);
        else
          fprintf(fp, "       ");
      } else if ((chr = find_type(tree->data->indi, CHR)) &&
                 (date = find_type(chr, DATE))) {
        if ((chr_date = return_year(date->data))) // Yes one ='s
          fprintf(fp, " <b>c.</b>%4.4d", chr_date);
        else
      ...
      if ((death = find_type(tree->data->indi, DEAT)) &&
          (date = find_type(death, DATE))) {
        if ((d_date = return_year(date->data))) // Yes one ='s
          fprintf(fp, " <b>d.</b>%4.4d", d_date);
        else
          fprintf(fp, "       ");
      ...
      fprintf(fp,
              " <a "
              "href=\"/ruby/gedrelay.rbx?type=TA&depth=2&target=%s\"><b>Anc.</"
              "b></a>",
              indi_reference);
      fprintf(fp,
              " <a "
              "href=\"/ruby/gedrelay.rbx?type=TD&depth=2&target=%s\"><b>Des.</"
              "b></a>",
              indi_reference);

      // output Marriages.
      if ((fams = find_type(tree->data->indi, FAMS))) {
        // char spouse_fnames[LINE_LIMIT], spouse_sname[LINE_LIMIT];
        mcount = 1;
        spouse_name = ((spouse = find_spouse(fams, indi_reference)) &&
                       (name = find_type(spouse, NAME)))
                          ? name->data
        ...
        fprintf(fp,
                " <A "
                "href=\"/ruby/gedrelay.rbx?type=html&target=%s\"><b>M%d.</b></"
                "A> %s",
                fam_reference, mcount++, spouse_name);
...
void dump_Surname_as_html(FILE *fp, char letter, int no_surname) {
  ...
  fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=surnamef&target=index_?\" "
              ">?</A> <b>No Surname INDEX</b><p>\n");
  // fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=index&target=index_%c\">Full
  // Index</A>\n", letter);
  fprintf(fp, "<p>\n");
...
void dump_index_as_html(FILE *fp, char letter, int no_surname) {
  ...
  for (j = 0; j < 26; j++) // Splash indexes across top of the page
    fprintf(
        fp,
        "<A HREF=\"/ruby/gedrelay.rbx?type=index&target=index_%c\"  >%c</A> \n",
        j + 'A', j + 'A');
  fprintf(fp,
          "<A HREF=\"/ruby/gedrelay.rbx?type=index&target=index_?\"  >?</A>\n");
  fprintf(fp,
          "<A HREF=\"/ruby/gedrelay.rbx?type=surname&target=index_%c\"  >To "
          "Surname Only Index</A><br>\n",
          letter);

  for (j = 0; j < 26; j++) // Splash no surname indexes across top of the page
    fprintf(fp,
            "<A HREF=\"/ruby/gedrelay.rbx?type=indexf&target=index_%c\"  "
            ">%c</A> \n",
            j + 'A', j + 'A');
  fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=indexf&target=index_?\"  "
              ">?</A> <b>No Surname INDEX</b><p>\n");
  ...
  fprintf(fp, "</KBD></pre><b>Go to the Tree's </b><A "
              "HREF=\"https://www.burrowes.org/FamilyTree/\" ><b>Entry "
              "Page</b></A>.<p>\n");
  fprintf(fp, "<hr>Maintained by <A HREF=\"mailto:rob@cs.auckland.ac.nz\">Rob "
              "Burrowes</A>.<br>\n");
...
void dump_t_index_as_html(FILE *fp, btree_node_p r) {
  ...
  // fprintf(fp, "</KBD></pre><b>Go to the Tree's </b><A
  // HREF=\"https://www.burrowes.org/FamilyTree/\" ><b>Entry
  // Page</b></A>.<p>\n"); fprintf(fp,"<hr>Maintained by <A
  // HREF=\"mailto:rob@cs.auckland.ac.nz\">Rob Burrowes</A>.<br>\n");
  // fprintf(fp, "Rob's <A HREF=\"https://www.burrowes.org/~rob/\" >Home
  // Page</A>\n"); fprintf(fp, "</body></html>\n");
}

void dump_shortindex_as_html(FILE *fp, char *thesurname, int no_surname) {
  int j;
  char name_buff[LINE_LIMIT];
  ...
  for (j = 0; j < 26; j++) // Splash no surname indexes across top of the page
    fprintf(fp,
            ...
            j + 'A', j + 'A');
  fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=surnamef&target=index_?\" "
              ">?</A> <b>No Surname INDEX</b>\n");

  // fprintf(fp, "<A HREF=\"/ruby/gedrelay.rbx?type=index&target=index_%c\"
  // >Full Index</A>\n", letter);

  fprintf(fp, "<p>\n<pre><KBD>\n");
  sprintf(buff, "index%s_%c.html", s, letter);
  if (no_surname == 0 && p == 0 && name_buff[0] != '\0')
    dump_tree_body(fp, root[char_to_index(name_buff[0])], buff, 0x6, name_buff,
                   no_surname);
