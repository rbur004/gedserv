		...
		if(name = find_type(g, NAME))
		{
			do //Add under all names person has gone by.
			{
				...
				{		
					...
					fprintf(fp, "%-16.16s <A href=\"/ruby/gedrelay.rbx?type=html&target=%s\"  >%-24.24s</a>",
					...
					if(famc = find_type(g, FAMC))
					{
						...
						fprintf(fp, " <A href=\"/ruby/gedrelay.rbx?type=html&target=%s#%s\"  ><b>Fam.</b></A>",  buff, buff2);
					}
					else
						fprintf(fp, " <b>      </b>");
					...
					strip_ats(buff, g->data);
					fprintf(fp, " <a href=\"/ruby/gedrelay.rbx?type=TA&depth=2&target=%s\"  ><b>Anc.</b></a>  ", buff);
					fprintf(fp, "<a href=\"/ruby/gedrelay.rbx?type=TD&depth=2&target/%s\"  ><b>Des.</b></a>  ", buff);
